...
第八个域：stealstolen(0)   which is the time spent in other operating systems
when running in a virtualized environment(since 2.6.11)
第九个域：guest(0)         which is the time spent running a virtual  CPU  for
guest operating systems under the control of the Linux kernel(since 2.6.24)
    ...
    def get_now_data(self):
        ...
        @Return: (status, msgs, results)
            ...
            results = DICT {
                ...
                除IO等待时间以外的其它等待时间
                "total": 755982, #总的cpu时间totalCpuTime = user + nice +
                system + idle + iowait + irq + softirq + stealstolen  +  guest
            }
        """
        # 获取当前数据
        try:
